...
registered_users = set() # Bot yonganda bazadan unikal ID'larni yuklab olish uchun
...
def setup_google_sheets():
    try:
        ...
        # Unikal foydalanuvchilarni xotiraga yuklab olish (ID'lar 2-ustunda deb hisoblasak)
        try:
            ids = sheet.col_values(2)[1:] # Sarlavhadan pastki ID'lar
            for uid in ids: registered_users.add(str(uid))
        except: pass
...
def log_user_uniquely(user: types.User, faculty, group):
    ...
    if uid not in registered_users:
        if sheet_instance:
            try:
                ...
                sheet_instance.append_row([now, uid, username, user.full_name, faculty, group])
                registered_users.add(uid)
                logging.info(f"🆕 Yangi foydalanuvchi qo'shildi: {uid}")
            except Exception as e:
                logging.error(f"Loglashda xato: {e}")
...
async def group_select(callback: types.CallbackQuery):
    ...
    # Unikal loglash
    log_user_uniquely(callback.from_user, fak, group)
    ...
    if url in screenshot_cache and (now - screenshot_cache[url]['time']) < 3600:
        sent_pic = await bot.send_photo(chat_id, screenshot_cache[url]['file_id'], caption=f"✅ {group}", reply_markup=kb.as_markup())
    ...
    try:
        ...
        sent_pic = await bot.send_photo(chat_id, types.FSInputFile(filename), caption=f"✅ {group}", reply_markup=kb.as_markup())
...
async def main():
    logging.info("🚀 Bot is running with unique logging!")
